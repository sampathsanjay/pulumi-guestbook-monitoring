...
Prometheus and Grafana were integrated using the kube-prometheus-stack Helm chart.
...
Frontend pods were configured with Prometheus annotations:
...
```bash
kubectl describe pod $(kubectl get pod -l app=frontend -o jsonpath='{.items[0].metadata.name}') | grep prometheus
```
