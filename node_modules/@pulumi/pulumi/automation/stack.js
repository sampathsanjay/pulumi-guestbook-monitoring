...
Event: ${line}\n${e.toString()}`);
     ...
     */
    async cancel() {
        await this.run((api, base) => api.cancel({
            ...base,
            stack: this.name,
        }));
    }
     ...
     */
    createBaseOptions(onOutput, onError, signal) {
     ...
     */
    async run(build, onOutput, onError, signal) {
        const ws = this.workspace;
        const base = this.createBaseOptions(onOutput, onError, signal);
        const result = await build(ws.cliApi, base);
